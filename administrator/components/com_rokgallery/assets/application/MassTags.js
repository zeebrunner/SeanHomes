/*
 * @version   $Id: MassTags.js 10876 2013-05-30 06:23:01Z btowles $
 * @author    RocketTheme http://www.rockettheme.com
 * @copyright Copyright (C) 2007 - 2015 RocketTheme, LLC
 * @license   http://www.gnu.org/licenses/gpl-2.0.html GNU/GPLv2 only
 */
((function(){this.MassTags=new Class({Implements:[Options,Events],options:{url:""},initialize:function(a){this.setOptions(a);
this.request=new Request({url:this.options.url,onSuccess:this.success.bind(this)});["addTags","removeTags","getTagPopup"].each(this.actions.bind(this,false));
},success:function(a){if(!JSON.validate(a)){return this.error("Invalid JSON response. ",a);}a=JSON.decode(a);if(a.status!="success"){return this.error(a.message);
}this.fireEvent(this.action,[a,this.id]);this.done(a);return this;},start:function(){this.fireEvent("start",this.id);},done:function(a){this.fireEvent("done",[this.id,a]);
},error:function(a){this.fireEvent("error",[a,this.action]);},actions:function(a,b){this[b]=function(c){this.start();this.fireEvent("before"+b.capitalize());
this.action=b;var d={model:"files",action:b};if(c){d.params=JSON.encode(c);}else{d.params=JSON.encode({});}this.request.send({data:d});return this;};}.protect()});
this.MassTagsManager=new Class({Extends:MassTags,options:{url:"",onGetTagPopup:function(a){window.Popup.statusBar.getElement(".loading").setStyle("display","none");
