/*
 * @version   $Id: Popup.js 10876 2013-05-30 06:23:01Z btowles $
 * @author    RocketTheme http://www.rockettheme.com
 * @copyright Copyright (C) 2007 - 2015 RocketTheme, LLC
 * @license   http://www.gnu.org/licenses/gpl-2.0.html GNU/GPLv2 only
 */
((function(){this.Popup=new Class({Implements:[Options,Events],options:{popup:"popup",overlay:"overlay"},initialize:function(a){this.setOptions(a);
this.bounds=this.setBounds();this.popup=document.id(this.options.popup).inject(document.body);this.overlay=document.id(this.options.overlay).inject(document.body);
this.topBar=this.popup.getElement(".topbar");this.content=this.popup.getElement(".content");this.statusBar=this.popup.getElement(".statusbar");this.setEvents();
},setBounds:function(){return{popup:{open:this.showPopup.bind(this),close:this.hidePopup.bind(this),ok:this.continuePopup.bind(this)},overlay:{show:this.showOverlay.bind(this),hide:this.hideOverlay.bind(this)}};
...
}else{document.getElements("html, body").setStyle("height","auto");}this.content.set("class","content clearfix");this.overlay.fireEvent("show");this.popup.fireEvent("open");
