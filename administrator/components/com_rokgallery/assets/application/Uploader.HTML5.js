...
window.Popup.counter.getElement("span").set("text",this.files.length);this.fireEvent("fileRemove");},dragover:function(c){if(!c.dataTransfer){c.dataTransfer=c.event.dataTransfer;
}c.stop();c.dataTransfer.dropEffect=(this.running)?"none":"copy";return this.fireEvent("dragOver");},dragout:function(c){return this.fireEvent("dragOut");
},drop:function(c){if(!c.dataTransfer){c.dataTransfer=c.event.dataTransfer;}c.stop();if(this.running){return false;}this.enqueue(c.dataTransfer.files);
