var Rainbows=[];var MooRainbow=new Class({Implements:[Options,Events],options:{id:"mooRainbow",prefix:"moor-",imgPath:"images/",startColor:[255,0,0],wheel:false,transparent:1,onComplete:function(){},onChange:function(){}},initialize:function(c,d){this.element=document.id(c);
...
h=f.rgbToHex();}else{if(i=="hsb"){g=f;j=f.hsbToRgb();h=j.rgbToHex();}else{h=f;j=f.hexToRgb();g=j.rgbToHsb();}}this.sets={rgb:j,hsb:g,hex:h};if(this.pickerPos.x!=null){this.autoSet(g);
...
h=this.snippet("curSize","int").h;e=this.snippet("curSize","int").w;f=Array.from(this.arrRGB).concat(this.arrHSB,this.hexInput);document.addEvent("click",function(){if(this.visible){this.hide(this.layout);
}}.bind(this));f.each(function(a){a.addEvent("keydown",this.eventKeydown.bind(this,a));a.addEvent("keyup",this.eventKeyup.bind(this,a));},this);[this.element,this.layout].each(function(a){a.addEvents({click:function(b){if(b){b.stop();
}},keyup:function(b){if(b.key=="esc"&&this.visible){this.hide(this.layout);}}.bind(this)},this);},this);g={x:[0-e,(this.layout.overlay.width-e)],y:[0-h,(this.layout.overlay.height-h)]};
this.layout.drag=new Drag(this.layout.cursor,{limit:g,onBeforeStart:this.overlayDrag.bind(this),onStart:this.overlayDrag.bind(this),onDrag:this.overlayDrag.bind(this),snap:0});
this.layout.overlay2.addEvent("mousedown",function(a){this.layout.cursor.setStyles({top:a.page.y-this.layout.overlay.getTop()-h,left:a.page.x-this.layout.overlay.getLeft()-e});
...
this.layout.slider.addEvent("mousedown",function(a){this.layout.arrows.setStyle("top",a.page.y-this.layout.slider.getTop()+this.snippet("slider")-d);this.layout.sliderDrag.start(a);
}.bind(this));},sliderDrag:function(){var d=this.snippet("arrSize","int"),c;this.sliderPos=this.snippet("arrPos")-d;this.setMooRainbow(this.parseColors(this.pickerPos.x,this.pickerPos.y,this.sliderPos),"hsb");
...
this.fireEvent("onChange",[this.sets,this]);}.bind(this));},wheelEvents:function(){var b=Array.from(this.arrRGB).extend(this.arrHSB);b.each(function(a){a.addEvents({mousewheel:this.eventKeys.bind(this,a),keydown:this.eventKeys.bind(this,a)});
},this);[this.layout.arrows,this.layout.slider].each(function(a){a.addEvents({mousewheel:this.eventKeys.bind(this,[this.arrHSB[0],"slider"]),keydown:this.eventKeys.bind(this,[this.arrHSB[0],"slider"])});
...
}},eventKeyup:function(k,n){var j=k.code,i=k.key,m,l,e=n.value.charAt(0);if(n.value!=null){return;}if(n.className.test(/hexInput/)){if(e!="#"&&n.value.length!=6){return;
...
}this.manualSet(m,"hsb");this.fireEvent("onChange",[this.sets,this]);}else{m=n.value.hexToRgb(true);if(isNaN(m[0])||isNaN(m[1])||isNaN(m[2])){return;}if(m!=null){this.manualSet(m);
...
var I=L.clone().inject(G).addClass(F+"BrighInput");L.inject(H).addClass(F+"HueInput");B.appendText(" %");G.appendText(" %");new Element("span",{styles:{position:"absolute"},"class":F+"ballino"}).set("html"," &deg;").inject(H,"after");
