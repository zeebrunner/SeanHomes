...
class RSFormProHelper
{
	...
	public static function showForm($formId, $val=array(), $validation=array())
	{
		...
		array_push($find, '{global:formid}', '{global:formtitle}', '{global:username}', '{global:userip}', '{global:userid}', '{global:useremail}', '{global:fullname}', '{global:sitename}', '{global:siteurl}');
		array_push($replace, $form->FormId, $form->FormTitle, $user->get('username'), isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '', $user->get('id'), $user->get('email'), $user->get('name'), $jconfig->get('sitename'), JURI::root());
		...
		if (empty($invalid))
		{
			...
			if (isset($files['form']['tmp_name']) && is_array($files['form']['tmp_name']))
			{
				...
				foreach ($files['form']['tmp_name'] as $fieldName => $val)
				{
					...
					// Upload File
					if (JFile::upload($files['form']['tmp_name'][$fieldName], $file)) {
