...
class Sh404sefClassRouterInternal extends JRouterSite
{
	...
	protected function _checkAliases($uri)
	{
		...
		$path = $uri->getPath();
		$requestPath = str_replace(JURI::root() . ltrim($config->shRewriteStrings[$config->shRewriteMode], '/'),
	...
	{
		// J! 3.4.3 language filter was broken, not reading cookie language when
		...
		)
		{
			...
			{
				$path = Sh404sefHelperLanguage::getDefaultLanguageSef() . '/';
				$uri->setPath($path);
		...
		if (function_exists('shFindSimilarUrls'))
		{
			...
			if (!empty($urlList))
			{
				...
				if (!empty($target->oldurl) && $path != $target->oldurl && $this->_canRedirectFrom($uri, $method = '', $path))
				{
					...
					if (!empty($message))
					{
						JFactory::getSession()->set('wb_sh404sef_404_redirect_message',
							array(
								'content'          => $message,
	...
	protected function _checkShurls($uri)
	{
		...
		if ($config->enablePageId)
		{
			try
			{
				$shurlRecord = ShlDbHelper::selectObject('#__sh404sef_pageids', '*', array(
					'pageid' => $path
				));
