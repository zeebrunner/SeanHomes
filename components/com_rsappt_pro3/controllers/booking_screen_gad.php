...
//		$this->registerTask( 'braintree_submit', 'braintree_submit' );
			...
			for($stci=0; $stci<$seat_type_count; $stci++){
				...
				if($seat_type_qty > 0){
					...
					try{					
						...
						logIt($e->getMessage(), "ctrl_booking_screen_gad", "", "");
						echo JText::_('RS1_SQL_ERROR');
						exit;
					}
				}
...
//						$_POST['user_field'.$i.'_udf_id'],
		...
		if(($pay_proc_submit != "0" && $pay_proc_submit != "4" && $pay_proc_submit != "4b") && floatval($grand_total) > 0){
			/* ----------------------------------------------------------------------------------- 
			...
			if($facebook == "Yes"){				
//				$this->setRedirect( JRoute::_( 'index.php?option=com_rsappt_pro3&view=booking_screen_gad&format=fb&Itemid='.$frompage_item.'&task='.$next_view.'&req_id='.$last_id->last_id));
				...
				if($seo == "1"){		
					$url = JRoute::_( 'index.php?option=com_rsappt_pro3&view=booking_screen_gad&Itemid='.$frompage_item.'&task='.$next_view.'&req_id='.$last_id->last_id.($tmpl!=""?"&tmpl=".$tmpl:""));
				} else {
					$url = 'index.php?option=com_rsappt_pro3&view=booking_screen_gad&Itemid='.$frompage_item.'&task='.$next_view.'&req_id='.$last_id->last_id.($tmpl!=""?"&tmpl=".$tmpl:"");
				}
...
//	function braintree_submit(){
//		include_once(JPATH_SITE.'/components/com_rsappt_pro3/payment_processors/braintree/braintree_goto2.php');
//	}
