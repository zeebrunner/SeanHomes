...
//		}
	...
	//=========================================================================
		...
		$resource = $jinput->getString('resources');
		...
		try{
			...
			logIt($e->getMessage(), "booking_screen_simple", "", "");
			...
			for($stci=0; $stci<$seat_type_count; $stci++){
				...
				if($seat_type_qty > 0){
					...
					try{
						...
						logIt($e->getMessage(), "ctrl_booking_screen_simple", "", "");
						echo JText::_('RS1_SQL_ERROR');
						exit;
					}
				}
...
//						$_POST['user_field'.$i.'_udf_id'],
		...
		if(($pay_proc_submit != "0" && $pay_proc_submit != "4") && floatval($grand_total) > 0){
			...
			if($pay_proc_submit != "0" && $pay_proc_submit != "4"){
				...
				$from_screen = "booking_screen_simple";
			...
			$url = 'index.php?option=com_rsappt_pro3&view=booking_screen_simple&Itemid='.$frompage_item.'&task='.$next_view.'&req_id='.$last_id->last_id.($tmpl!=""?"&tmpl=".$tmpl:"");
			if($seo == "1"){		
