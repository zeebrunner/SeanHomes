<?php
/**
 * @version $Id: rokcandy.php 25395 2015-01-14 15:34:50Z james $
...
class plgSystemRokCandy extends JPlugin {
	...
	function _shouldProcess() {
	    ...
	    //don't run if in search mode
	    if (JFactory::getApplication()->input->get('option') == "com_search" && JFactory::getApplication()->input->get('type') == "component")
            return true;
		
	    //don't run if in edit mode and flag enabled
	    if (JFactory::getApplication()->input->get('layout') == 'edit' && JFactory::getApplication()->input->get('type') == "component" && $params->get('editenabled',0) == 0)
            return true;
