...
class GantryDropdownLayout extends AbstractRokMenuLayout
{
    ...
    protected function renderItem(JoomlaRokMenuNode &$item, RokMenuNodeTree &$menu)
    {
		...
		if($custom_class == 'guestsonly' && isset($_COOKIE['UBAM'])) return;
        ...
        if ($dropdown_width == 0) $dropdown_width = 'auto';

        $wrapper_css = ' style="width:'.$dropdown_width.';"';
       ...
       if ($child_type!="menuitems") {
        ...
        ?>
        <li
		<?php if($item->haslistitemclasses()) : ?>class="<?php echo $item->getListItemClasses()?>"<?php endif;?>
		<?php if($item->hascssid() && $this->activeid):?>id="<?php echo $item->getCssId();?>"<?php endif;?>
		>
			 <a
			 <?php if($item->haslinkclasses()):?>class="<?php echo $item->getLinkClasses();?>"<?php endif;?>
			 <?php if($item->haslink()):?>href="<?php echo $item->getLink();?>"<?php endif;?>
			 <?php if($item->hasTarget()):?>target="<?php echo $item->getTarget();?>"<?php endif;?>
			 <?php if ($item->hasAttribute('onclick')): ?>onclick="<?php echo $item->getAttribute('onclick'); ?>"<?php endif; ?>
			 <?php if ($item->hasLinkAttribs()): ?> <?php echo $item->getLinkAttribs(); ?><?php endif; ?>
			 >
                ...
                <?php
					if ($custom_icon && $custom_icon != -1) {
						...
						$link_name = explode(' ',trim($item->getListItemClasses()));
						switch ($link_name[0]) {
							case "item101":
								echo '<span class="icon-home"></span>';
								break;
							case "item149-contact":
								echo '<span class="icon-envelope-alt"></span>';
								break;
							case "item150":
								echo '<span class="icon-headphones"></span>'.'<span class="menulabel">'.$item->getTitle().'</span>';
								break;
							default:
							   echo $item->getTitle();
						}						
